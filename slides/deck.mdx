...
<Notes>
  cDU fired on each render, checks if field changed, changes layout of keyboard or manages title
  case.
</Notes>
...
<Notes>Last keystroke and hit tab, switch title fires and goes from number back to text</Notes>
...
<Notes>
  useEffect similar to componentDidMount, componentDidUpdate, and componentWillUnmount combined. No
  need to think about timeouts in keypress. Allow component lifecycle to handle those parts.
</Notes>
...
# Special Sauce in Three Flavours
...
3. Share logic with Hooks, comapre to class
